...
INPUT_DIR="/global/home/hpc3862/ForGithub/"
# ChIP name
CHIP_NAME="a.bed"
# Control name
CONTROL_NAME="b.bed"
...
if [ $BOOTSTRAP -eq 1 ]
then
	perl $PERL_PATH --dirOrig "$OUTPUT_DIR/SICER_original" --nameOrig "${CHIP_NAME%.bed}-W${WINDOW}-G${GAP}-islands-summary" --dirRemix "$OUTPUT_DIR/SICER_re-mix" --nameRemix "${CHIP_NAME%.bed}" --dirOutput "$OUTPUT_DIR/SICER_RECAP" --nameOutput "${CHIP_NAME%.bed}.RECAP.bootstrap_${i}-W${WINDOW}-G${GAP}-islands-summary" --bootstrap 1 --header $HEADER --pvalCol 6 --delim t --software O
else
	for (( i=1; i<=$BOOTSTRAP; i++ ))
	do
