...
if [ $BOOTSTRAP -eq 1 ]
then
	perl $PERL_PATH --dirOrig "$OUTPUT_DIR/diffReps_original" --nameOrig "${CHIP_NAME%.bed}.txt" --dirRemix "$OUTPUT_DIR/diffReps_re-mix" --nameRemix "${CHIP_NAME%.bed}.bootstrap_$i.txt" --dirOutput "$OUTPUT_DIR/diffReps_RECAP" --nameOutput "${CHIP_NAME%.bed}.RECAP.bootstrap_$i.txt" --bootstrap 1 --header $HEADER --pvalCol 13 --delim t --software D
else
